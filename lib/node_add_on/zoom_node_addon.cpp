...
void ZoomNodeWrap::SetSupportDarkModel(const v8::FunctionCallbackInfo<v8::Value>& args) {
	v8::Isolate* isolate = args.GetIsolate();
	
	if (args.Length() < 1) {
		isolate->ThrowException(v8::Exception::TypeError(
			v8::String::NewFromUtf8(isolate, "ZoomNodeWrap::SetSupportDarkModel-Wrong number of arguments", v8::NewStringType::kInternalized).ToLocalChecked()));
		return;
	}

    bool isDark = false;
	zoom_v8toc(args[0].As<v8::Boolean>(), isDark);

	ZNSDKError err = _g_native_wrap.SetSupportDarkModel(isDark);
